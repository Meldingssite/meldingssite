...
function renderDropDown(dropObj) {
    ...
    dropHTML += "<section class=input title = '" + dropObj.text + "'><select name='" + dropObj.name + "'";
    if (dropObj.target != null || dropObj !== "") {
        dropHTML += "onChange=checkDropdown(this); class ='" + dropObj.target + "'"
    }
    dropHTML += ">";//knop

    for (var option in options) {
        dropHTML += "<option  name='" + options[option].name + "'value ='" + options[option].name + "'";
        dropHTML += ">" + options[option].text + "</option>";
    } //closing tags
